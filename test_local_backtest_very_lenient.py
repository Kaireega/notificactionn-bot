#!/usr/bin/env python3
"""
Test script to run backtesting with local historical data and very lenient signal requirements.
"""
import asyncio
import sys
import os
...
async def run_backtest_with_local_data():
    ...
    print("🚀 Starting Backtest with Local Historical Data (Very Lenient Signals)")
    print("=" * 60)
    ...
    print("🔄 Running backtest with very lenient parameters...")
    result = await backtest_engine.run_backtest(
        historical_data=historical_data,
        start_date=start_date,
        end_date=end_date,
        parameters={
            'rsi_oversold': 45,  # Very lenient (was 30)
            'rsi_overbought': 55,  # Very lenient (was 70)
            'risk_percentage': 2.0,
            'macd_signal_threshold': 0.000001,  # Very sensitive (was 0.0001)
            'bollinger_threshold': 0.005,  # Very sensitive (was 0.1)
            'atr_multiplier': 2.0,
            'min_signal_strength': 0.1  # Very lenient (was 0.5)
        }
    )
