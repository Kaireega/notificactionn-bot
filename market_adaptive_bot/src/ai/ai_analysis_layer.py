...
Analyze this market and provide a trade recommendation. Consider:
...
{{
    ...
    "estimated_hold_time_minutes": int,
...
"""
    ...
    async def analyze_market(
        ...
        """Analyze market and generate trade recommendation using AI."""
        
        # Check rate limiting
        if not self._can_analyze(pair):
            self.logger.info(f"Rate limited for {pair}, using cached analysis")
            return self._analysis_cache.get(pair)
        ...
        try:
            ...
            # Get appropriate prompt for market condition
            ...
            if recommendation and recommendation.confidence >= self.config.ai_confidence_threshold:
                # Cache the recommendation
                self._analysis_cache[pair] = recommendation
                self._last_analysis_time[pair] = datetime.utcnow()
                
                self.logger.info(f"Generated AI recommendation for {pair}: {recommendation.signal} "
                               f"(confidence: {recommendation.confidence:.2f})")
                
                return recommendation
            else:
                self.logger.info(f"AI recommendation below confidence threshold for {pair}")
                return None
                
        except Exception as e:
            self.logger.error(f"Error in AI analysis for {pair}: {e}")
            return None
    ...
    def _parse_ai_response(self, data: Dict[str, Any], pair: str) -> Optional[TradeRecommendation]:
        ...
        try:
            ...
            recommendation = TradeRecommendation(
                ...
                estimated_hold_time=timedelta(minutes=int(data.get("estimated_hold_time_minutes", 30)))
            )
    ...
    async def analyze_multiple_timeframes(
        ...
        try:
            ...
            for timeframe, candles in candles_by_timeframe.items():
                if candles:
                    recommendation = await self.analyze_market(pair, candles, market_context, timeframe)
            ...
            return self._generate_consensus_recommendation(recommendations, pair)
