...
def run_collection(ic: InstrumentCollection, api: OandaApi):
    # Only process these specific pairs
    target_pairs = ["EUR_USD", "USD_JPY", "GBP_JPY"]
    
    for pair in target_pairs:
        if pair in ic.instruments_dict.keys():
            for granularity in ["M5", "M15", "H1", "H4"]:
                print(f"Processing {pair} {granularity}")
                collect_data(
                    pair,
                    granularity,
            ...
            print(f"Warning: {pair} not found in available instruments")
