...
"""
    ...
    async def analyze_market(
        ...
        if not self._can_analyze(pair):
            self.logger.debug(f"⏰ Rate limit: Skipping analysis for {pair} on {timeframe}")
            return None
        ...
        try:
            ...
            if recommendation:
                ...
                self.logger.info(f"🎯 AI Recommendation for {pair} ({timeframe}): "
    ...
    async def get_market_sentiment(self, pair: str) -> float:
        """Get market sentiment for a pair (placeholder for future implementation)."""
        # This could integrate with news APIs, social sentiment, etc.
        return 0.0  # Neutral sentiment
    
    async def _determine_market_context(
        self, 
        pair: str, 
        candles: List[CandleData], 
        technical_indicators: TechnicalIndicators
    ) -> MarketContext:
        """Determine market context based on technical indicators and price action."""
        try:
            if len(candles) < 20:
                return MarketContext(condition=MarketCondition.UNKNOWN)
            
            # Calculate volatility (ATR-based)
            volatility = technical_indicators.atr if technical_indicators.atr else 0.0
            
            # Calculate trend strength
            trend_strength = 0.0
            if technical_indicators.ema_fast and technical_indicators.ema_slow:
                # Simple trend strength based on EMA separation
                ema_diff = abs(technical_indicators.ema_fast - technical_indicators.ema_slow)
                trend_strength = min(1.0, ema_diff / 0.001)  # Normalize
            
            # Determine market condition based on indicators
            condition = self._classify_market_condition(candles, technical_indicators)
            
            # Get key levels
            key_levels = {}
            if technical_indicators.support_level:
                key_levels['support'] = technical_indicators.support_level
            if technical_indicators.resistance_level:
                key_levels['resistance'] = technical_indicators.resistance_level
            if technical_indicators.bollinger_upper:
                key_levels['bb_upper'] = technical_indicators.bollinger_upper
            if technical_indicators.bollinger_lower:
                key_levels['bb_lower'] = technical_indicators.bollinger_lower
            ...
            self.logger.error(f"Error determining market context for {pair}: {e}")
            return MarketContext(condition=MarketCondition.UNKNOWN)
    ...
    def _classify_market_condition(
        ...
        """Classify market condition based on technical indicators."""
        try:
            ...
            price_change = (recent_candles[-1].close - recent_candles[0].open) / recent_candles[0].open
            price_volatility = sum(abs(float(c.high - c.low) / float(c.low)) for c in recent_candles) / len(recent_candles)
            
            # Check for high volatility (potential news/reactionary)
            if indicators.atr and indicators.atr > 0.0015:  # High ATR
                return MarketCondition.NEWS_REACTIONARY
            
            # Check for reversal patterns
            if indicators.rsi_14:
                if indicators.rsi_14 < 20 or indicators.rsi_14 > 80:  # Extreme RSI
                    return MarketCondition.REVERSAL
                
                # Check for RSI divergence
                if self._check_rsi_divergence(candles, indicators.rsi_14):
                    return MarketCondition.REVERSAL
            
            # Check for breakout patterns
            if indicators.bollinger_upper and indicators.bollinger_lower:
                current_price = float(candles[-1].close)
                if current_price > indicators.bollinger_upper or current_price < indicators.bollinger_lower:
                    return MarketCondition.BREAKOUT
            
            # Check for ranging market
            if indicators.rsi_14 and 30 <= indicators.rsi_14 <= 70:
                if indicators.bollinger_upper and indicators.bollinger_lower:
                    current_price = float(candles[-1].close)
            ...
            self.logger.error(f"Error classifying market condition: {e}")
            return MarketCondition.UNKNOWN
    
    def _check_rsi_divergence(self, candles: List[CandleData], current_rsi: float) -> bool:
        """Check for RSI divergence (simplified version)."""
        try:
            if len(candles) < 20:
                return False
            
            # Simple divergence check: if price is making new highs but RSI is lower
            recent_highs = [float(c.high) for c in candles[-5:]]
            if max(recent_highs) == recent_highs[-1] and current_rsi < 70:
                return True
