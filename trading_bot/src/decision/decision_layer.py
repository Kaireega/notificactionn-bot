        ...
        try:
            ...
            if not self._should_process_recommendation(recommendation):
                self.logger.info(
                    ...
                    f"({self.config.technical_confidence_threshold})"
                )
                return None
            ...
            if self.trade_recorder:
                self.trade_recorder.record_complete_trade_decision(
                    ...
                    ai_outputs={},  # No AI outputs in technical-only mode
    ...
    def _should_process_recommendation(self, recommendation: TradeRecommendation) -> bool:
        ...
        if recommendation.confidence < self.config.technical_confidence_threshold:
            self.logger.debug(f"🔍 Confidence check failed for {recommendation.pair}: "
                            f"{recommendation.confidence:.2f} < {self.config.technical_confidence_threshold}")
            return False
