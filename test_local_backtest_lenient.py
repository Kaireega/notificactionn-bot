#!/usr/bin/env python3
"""
Test script to run backtesting with local historical data and more lenient signal requirements.
"""
import asyncio
import sys
import os
...
async def run_backtest_with_local_data():
    ...
    print("🚀 Starting Backtest with Local Historical Data (Lenient Signals)")
    print("=" * 60)
    ...
    # Run backtest with more lenient parameters
    print("🔄 Running backtest with lenient parameters...")
    result = await backtest_engine.run_backtest(
        ...
        parameters={
            ...
            'risk_percentage': 2.0,
            'macd_signal_threshold': 0.00005,  # More sensitive (was 0.0001)
            'bollinger_threshold': 0.05,  # More sensitive (was 0.1)
            'atr_multiplier': 2.0,
            'min_signal_strength': 0.25  # More lenient (was 0.5)
        }
    )
